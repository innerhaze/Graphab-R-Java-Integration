...
```{r echo=FALSE, fig.align='center', fig.cap="*Figure 1. Illustration of the data processing workflow. In Step 1, each species is assigned a unique ID based on its coordinates, as detailed in [this other repository](https://github.com/innerhaze/Assignment-of-Unique-Identifiers-to-Patches-in-Connectivity-Modeling-Using-Graphab?tab=readme-ov-file). In Step 2, described in the 'Linksets in New Folder' section of this repository, connectivity indices are obtained for the IDs of each patch, corresponding to each species. Step 3, outlined in the 'Assignment of Species Identities to Linkset Results' section of this repository, involves renaming the IDs in the linkset files created in Step 2 using the IDs from Step 1.*", out.width='75%'}
